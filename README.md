...
For RWX, the controller first creates a controller-owned Recreate bootstrap
consumer that mounts the RWX PVC, which triggers Longhorn's share-manager. It
then discovers the share-manager Service/EndpointSlice and generates an
FSAL_PROXY_V4 helper using the pinned proxy image
